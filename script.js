...
const API_KEY = "b1343a498e75c9b956288e4984fb5f10"; // API key for OpenWeatherMap API
...
const renderInitialWeather = () => {
  const defaultCity = "Colombo"; // Specify the default city for initial rendering
  const API_URL = `https://api.openweathermap.org/geo/1.0/direct?q=${defaultCity}&limit=1&appid=${API_KEY}`;

  fetch(API_URL)
    .then((response) => response.json())
    .then((data) => {
      if (!data.length) return alert(`No coordinates found for ${defaultCity}`);
      const { lat, lon, name } = data[0];
      getWeatherDetails(name, lat, lon);
    })
    .catch(() => {
...
let showAllForecast = false;

viewMoreButton.addEventListener("click", () => {
  showAllForecast = !showAllForecast;
  const forecastCards = document.querySelectorAll(
    ".forecast-cards-wrapper li.card"
  );
